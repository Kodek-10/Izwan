...
function ConstellationView() {
    ...
    const fetchData = async () => {
      try {
        ...
        snippets.forEach((s: any) => {
          ...
          // Process tags
          const tags = s.tags?.map((t: any) => typeof t === 'string' ? t : t.name) || [];
          tags.forEach((tag: string) => {
            ...
            if (!tagMap.has(tagId)) {
              const newTagNode = {
                id: tagId,
                name: `#${tag}`,
                val: 3, // Smaller size for tags
                color: "#a855f7", // Purple neon color for tags
                type: "tag"
              };
            ...
            // Create link from tag to snippet
            links.push({
              source: tagId,
  ...
  useEffect(() => {
    ...
    // We check if it's dark mode, but we will force the background to #1a1a2e if it is dark mode
    ...
    const zoom = d3.zoom<SVGSVGElement, unknown>()
      ...
      .on("zoom", (event) => {
        ...
        // Scale text size inversely to zoom to maintain visual legibility
        g.selectAll(".node-label")
          .attr("font-size", (d: any) => {
            const baseSize = d.type === "tag" ? 10 : 12;
            // Cap the scale so it doesn't get infinitely small or huge
            const effectiveScale = Math.max(0.3, Math.min(scale, 3));
            return `${baseSize / effectiveScale}px`;
          })
          .attr("dy", (d: any) => {
            const baseDy = d.type === "tag" ? d.val * 1.2 + 14 : d.val * 1.5 + 18;
            const effectiveScale = Math.max(0.3, Math.min(scale, 3));
    ...
    const simulation = d3.forceSimulation(graphData.nodes)
      .force("link", d3.forceLink(graphData.links).id((d: any) => d.id).distance(20))
      .force("charge", d3.forceManyBody().strength(-80))
      .force("center", d3.forceCenter(width / 2, height / 2))
      .force("radial", d3.forceRadial(50, width / 2, height / 2).strength(0.1))
      .force("collide", d3.forceCollide().radius((d: any) => d.val * 1.5 + 8).iterations(3));
    ...
    const nodeGroup = g.append("g")
      ...
      .on("click", (event, d: any) => {
        if (d.type === "snippet") {
          window.location.href = `/snippets/${d.rawId}`;
        }
      })
    ...
    nodeGroup.append("circle")
      ...
      .style("cursor", (d: any) => d.type === "snippet" ? "pointer" : "default");
    ...
    nodeGroup.append("text")
      ...
      .attr("dy", (d: any) => d.type === "tag" ? d.val * 1.2 + 14 : d.val * 1.5 + 18)
      .attr("text-anchor", "middle")
      .text((d: any) => d.name)
      .attr("fill", (d: any) => isDark ? (d.type === "tag" ? "#e9d5ff" : "#ffffff") : (d.type === "tag" ? "#7e22ce" : "#000000"))
      .attr("font-size", (d: any) => d.type === "tag" ? "10px" : "12px")
      .attr("font-weight", (d: any) => d.type === "tag" ? "600" : "500")
      .attr("font-family", "system-ui, sans-serif")
      .style("pointer-events", "none");

    // Adjust text background size based on text width
    ...
    simulation.on("tick", () => {
      link.attr("d", (d: any) => {
        // Curve the line slightly so bidirectional links don't perfectly overlap
    ...
    function handleMouseOver(event: any, d: any) {
      ...
      currentNodeGroup.select("circle")
        .attr("r", (d: any) => d.type === "tag" ? d.val * 1.8 : d.val * 2.2)
        .style("stroke", "#ffffff")
        .style("stroke-width", "2px");

      // Highlight links where this node is the SOURCE (outgoing links) or TARGET
      // The prompt mentioned highlighting outgoing links "ses liens sortants".
      link
        .attr("stroke", (l: any) => {
          if (l.source.id === d.id) return isDark ? "#4ade80" : "#16a34a"; // Green for outgoing
          if (l.target.id === d.id) return isDark ? "#60a5fa" : "#2563eb"; // Blue for incoming
          return isDark ? "rgba(255, 255, 255, 0.05)" : "rgba(0, 0, 0, 0.05)";
        })
        .attr("stroke-width", (l: any) => {
          if (l.source.id === d.id || l.target.id === d.id) return Math.max(2, Math.sqrt(l.value) * 1.5);
          return Math.max(1, Math.sqrt(l.value));
        })
        .attr("marker-end", (l: any) => {
          if (l.source.id === d.id || l.target.id === d.id) return "url(#arrow-highlight)";
          return "url(#arrow)";
        });
  ...
  function getColorForLanguage(lang: string) {
    ...
    if (["javascript", "js", "typescript", "ts", "jsx", "tsx"].includes(l)) return "#f1e05a";
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Constellation</h1>
        <p className="text-muted-foreground">
          Visualisez les connexions entre vos snippets basées sur leurs tags. (D3.js)
        </p>
      </div>

      <div 
        ref={containerRef} 
        className="flex-1 border border-border rounded-xl overflow-hidden shadow-sm relative min-h-[500px]"
