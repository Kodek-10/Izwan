...
export const Route = createFileRoute("/_app/statistics")({
  ...
  loader: async () => {
    ...
    if (typeof window === "undefined") {
      return { pieData: [], lineData: [], totalCount: 0, topLanguages: [], recentActivity: [], needsClientFetch: true };
    }
    ...
    try {
      ...
      return { 
        pieData,
        lineData: last6Months,
        totalCount: snippets.length,
        topLanguages: pieData.sort((a, b) => b.value - a.value).slice(0, 4),
        recentActivity: snippets.slice(0, 5).map(s => ({
          ...s,
      ...
      return { pieData: [], lineData: [], totalCount: 0, topLanguages: [], recentActivity: [], needsClientFetch: false };
    }
  },
  component: StatsPage,
});
  ...
  if (isClientLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">Analyse de vos données en cours...</p>
      </div>
    );
  }

  const { pieData, lineData, totalCount, topLanguages, recentActivity } = stats;

  return (
    <div className="space-y-6 pb-10">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Snippets" value={totalCount} icon={<Code2 />} color="bg-blue-500" />
        <StatCard title="Langages" value={pieData.length} icon={<Globe />} color="bg-indigo-500" />
        <StatCard title="Activité (6m)" value={lineData.reduce((acc, curr) => acc + curr.count, 0)} icon={<LayoutDashboard />} color="bg-pink-500" />
        <StatCard title="Dernière Maj" value="Aujourd'hui" icon={<Clock />} color="bg-amber-500" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Language Distribution */}
        <div className="bg-card border border-border rounded-2xl p-4 sm:p-6 shadow-sm">
          <h3 className="font-display font-semibold text-base sm:text-lg mb-6">Répartition par langage</h3>
        ...
        {/* Creation Activity */}
        <div className="bg-card border border-border rounded-2xl p-4 sm:p-6 shadow-sm">
          <h3 className="font-display font-semibold text-base sm:text-lg mb-6">Activité de création</h3>
          <div className="h-64 sm:h-72">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 bg-card border border-border rounded-2xl p-5 sm:p-6 shadow-sm">
          <h3 className="font-display font-semibold text-base sm:text-lg mb-4">Top Langages</h3>
          <div className="space-y-4">
            {topLanguages.map((l, i) => (
              <div key={l.name} className="flex items-center justify-between">
                ...
                <span className="text-xs text-muted-foreground font-mono">{l.value} snippets</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-2xl p-4 sm:p-6 shadow-sm">
          <h3 className="font-display font-semibold text-base sm:text-lg mb-4">Activités Récentes</h3>
          <div className="space-y-3">
            {recentActivity.map((s) => (
              <div key={s.id} className="flex items-center justify-between p-2.5 sm:p-3 rounded-xl hover:bg-muted/50 transition-colors">
                ...
                <span className="text-[10px] sm:text-xs text-muted-foreground tabular-nums ml-2 shrink-0">{s.date}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
      <div className="flex items-center justify-between">
        <div className={`h-10 w-10 rounded-xl ${color} text-white flex items-center justify-center shadow-lg shadow-${color.split('-')[1]}-500/20`}>
          {icon}
        </div>
