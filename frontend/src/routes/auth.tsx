...
import { Eye, EyeOff, Loader2, Code, Lock, Mail, CheckCircle2 } from "lucide-react";
...
function readVscodeContext(): VscodeContext | null {
  ...
  const fromVscode = query.get("source") === "vscode" || hash.get("redirect_uri") != null;
  ...
  // On nettoie l'URL (le token ne doit pas rester dans la barre d'adresse).
  if (hash.toString()) {
...
function VscodeReturnScreen({
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center py-10">
      ...
      <h2 className="font-headline-lg text-3xl text-[var(--on-surface)] mb-2">
        Connexion réussie !
      </h2>
      <p className="text-[var(--on-surface-variant)] font-body-md mb-8 max-w-sm">
        Retournez maintenant dans votre éditeur VSCode pour finaliser la connexion de l'extension
        Izwan.
      </p>
      <button
        className="auth-btn-primary w-full"
        onClick={() => (window.location.href = vscodeUri)}
      >
        Retourner vers VSCode
      </button>
      <button
        type="button"
        className="mt-4 text-sm text-[var(--on-surface-variant)] hover:text-[var(--primary)] underline"
        onClick={onClose}
      >
...
function AuthPage() {
  ...
  useEffect(() => {
    // Flux extension VSCode : on capture le contexte (query au départ, fragment au
    // retour OAuth) et on empêche la redirection classique vers le dashboard.
    const vscodeContext = readVscodeContext();
    if (vscodeContext) {
      setVscode(vscodeContext);
      localStorage.setItem("izwan_auth", "1");
      return;
    }

    // Retour de l'écran OAuth : le backend a posé le cookie, on pose le drapeau
    // de présence puis on redirige selon le rôle.
    ...
    if (oauth === "success") {
      ...
      api
        ...
        .then((me) => {
          navigate({ to: me.role === "ADMIN" ? "/admin" : "/dashboard", replace: true });
        })
        .catch(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Flux extension VSCode : on garde le token pour l'afficher via le bouton
      // "Retourner vers VSCode" au lieu de rediriger vers le dashboard.
      if (vscode) {
        setVscode({ ...vscode, token: result.access_token });
        return;
      }
  ...
  // Flux extension : une fois connecté, on affiche l'écran de retour vers VSCode.
  if (vscode && vscode.token) {
    return (
      <main className="auth-page min-h-screen flex items-center justify-center bg-surface-container-lowest">
        <div className="w-full max-w-md px-6">
          <VscodeReturnScreen
            redirect_uri={vscode.redirect_uri}
            state={vscode.state}
            token={vscode.token}
            onClose={() => setVscode({ ...vscode, token: "" })}
          />
        </div>
      </main>
    );
      ...
      <section className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12 lg:p-24 bg-surface-container-lowest">
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-2">
            <h2 className="font-headline-lg text-3xl text-[var(--on-surface)]">
              Content de vous revoir
            </h2>
            <p className="text-[var(--on-surface-variant)] font-body-md text-lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              onClick={() =>
                api.oauthLogin(
                  "google",
                  vscode ? { redirect_uri: vscode.redirect_uri, state: vscode.state } : undefined,
                )
              }
            >
              ...
              <div className="group">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    placeholder="jean_dupont ou jean@exemple.com"
